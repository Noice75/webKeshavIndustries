import { getSales, getNextSales } from '../js/Firestore.js';
...
async function loadData() {
    ...
    while (index < docData.length) {
        let x = docData[index]
        docBody.innerHTML += `<tr><th scope="row">${index + 1}</th><td>${x['date']}</td><td>${x['nameOfCustomer']}</td><td>${x['typeOfRawMaterial']}</td><td>${x['quantity']}</td><td>${x['rate']}</td><td>${x['totalAmount']}</td><td>${x['gst']}</td><td>${x['totalGrossAmount']}</td></tr>`
        index++;
    }
...
async function loadMore() {
    ...
    var nextdocData;
    try {
        nextdocData = await getNextSales();
    }
    catch (err) {
        alert(err)
    }
...
async function applyDateFilter() {
    await decoyTable(false);
    const docData = await filterByDate('sales', document.getElementById('filter-date').value);
...
async function applyDateRangeFilter() {
    await decoyTable(false);
    const docData = await filterByDateRange('sales', document.getElementById('filter-startDate').value, document.getElementById('filter-endDate').value);
    if (docData == undefined) {
        await decoyTable();
        return
    }
...
async function applyYearFilter() {
    await decoyTable(false);
    const docData = await filterByYear('sales', document.getElementById("yearDateSelector").value);
    if (docData == undefined) {
        await decoyTable();
        return
    }
